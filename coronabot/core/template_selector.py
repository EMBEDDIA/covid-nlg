...
class TemplateSelector(NLGPipelineComponent):
    ...
    def _filter_by_context(
        ...
        if proposed:
            log.debug("Non-empty filtered list after time filter, checkpointing")
            templates = proposed

        # Filter s.t. location is either mandatory present or absent based on context
        if (
            context
            and context.main_fact.where == this.main_fact.where
            and context.main_fact.where_type == this.main_fact.where_type
        ):
            proposed = [template for template in templates if not template.has_slot_of_type("where")]
        else:
            proposed = [template for template in templates if template.has_slot_of_type("where")]
