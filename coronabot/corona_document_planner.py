...
class CoronaBodyDocumentPlanner(BodyDocumentPlanner):
...
class CoronaHeadlineDocumentPlanner(HeadlineDocumentPlanner):
    def select_next_nucleus(
        self, available_message: List[Message], selected_nuclei: List[Message]
    ) -> Tuple[Message, float]:
        return _select_next_nucleus(available_message, selected_nuclei)
...
def _select_next_nucleus(
    ...
    selected_analyses = [nucleus.main_fact.value_type.split(":")[0] for nucleus in selected_nuclei]
    log.debug("Already talked about {}".format(selected_analyses))

    available = [
        message for message in available_messages if message.main_fact.value_type.split(":")[0] not in selected_analyses
    ]
...
def _weigh_by_analysis_similarity(
    ...
    prev_value_type_fragments = previous.main_fact.value_type.split(":")
    for n, fragment_count in enumerate(reversed(range(len(prev_value_type_fragments)))):
        value_prefix = ":".join(prev_value_type_fragments[: fragment_count + 1])

        for score, message in messages:
            if message.main_fact.value_type.startswith(value_prefix):
...
def _weigh_by_context_similarity(
    ...
    for score, message in messages:
        if previous.main_fact.timestamp_type == message.main_fact.timestamp_type:
            if previous.main_fact.timestamp == message.main_fact.timestamp:
                score *= 1.5
            elif previous.main_fact.timestamp_type == "date_span":
                prev_from, prev_to = previous.main_fact.timestamp.split(":")
                this_from, this_to = message.main_fact.timestamp.split(":")
                if prev_from == this_from:
                    score *= 1.2
                if prev_to == this_to:
                    score *= 1.2
